...
- **Self-attention** — the mechanism where every position in a sequence attends to
  every other position, enabling direct information flow regardless of distance
- **Attention pattern/map** — the matrix of weights showing how much each token
  attends to every other token
- **Context window** — the set of tokens the model can attend to (and why it has limits)

#### Animation spec
- Full sentence displayed
- Click any word → see lines fanning out to all other words with varying thickness
- Thickness = attention weight (how much this word "looks at" each other word)
- Toggle between different "attention patterns" to show that different relationships
  emerge (syntactic, semantic, positional)
- Show the n² nature: 15 words = 225 attention pairs. 1000 words = 1,000,000 pairs.
  This is the first hint at the computational cost.

#### Bridge to Stop 3
"But HOW does the model decide how much attention to give each pair? It doesn't
compare raw words — it compares transformed versions of them. Those transformations
are Q, K, and V."
...
| 10 | Self-attention | Mechanism where every token attends to every other token in the sequence | Stop 2 |
| 11 | Attention pattern/map | Matrix of weights showing how much each token attends to every other | Stop 2 |
| 12 | Context window | The set of tokens the model can attend to | Stop 2 |
