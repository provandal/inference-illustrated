// Stop 15: Compressing the Cache — Making It Smaller
...
export const NARRATIONS = {
  cascading:
    'In our scenario, Llama-3 70B\u2019s KV cache takes 320 KB per token at FP16. For a 28,000-token conversation, that\u2019s 8.96 GB. If we could cut that in half \u2014 to 160 KB per token, 4.48 GB total \u2014 the effect cascades through every tier we saw in Stop 14: <strong>G1 (HBM)</strong> fits 288 active users instead of 144. <strong>G2 (DRAM)</strong> fits 1,600 warm conversations instead of 800. <strong>Transfer time:</strong> the P/D transfer from Stop 13 drops from 180 ms to 90 ms. <strong>Cache hit rate:</strong> more conversations maintained = fewer misses = fewer costly recomputes. <strong>Network bandwidth:</strong> every promotion/demotion moves half the data.\n\nCompression doesn\u2019t just save memory. It improves latency, throughput, network utilization, and cost at every level of the stack. That\u2019s why it\u2019s one of the most actively researched areas in LLM inference. There are three families of techniques, each attacking a different dimension of the cache.',

  architectural:
    'We introduced GQA in Stop 9 \u2014 grouping attention heads to share K and V. Now let\u2019s see all three architectural approaches in depth, with concrete cache sizes for our scenario.\n\nThese techniques are <strong>built into the model architecture</strong>. They\u2019re chosen during model design and training, not applied afterward. If a model was trained with GQA, you can\u2019t switch it to MHA or MLA later \u2014 the weight matrices were shaped for one specific approach.',
...
export const INFRA_IMPACT = [
  ...
  { component: 'Active users per GPU (Stop 12)',        fp16: '18',                      fp8: '36',                          improvement: '2\u00d7 more',              fp16Num: 18,   fp8Num: 36 },
  { component: 'Batch size capacity (Stop 12)',         fp16: '18',                      fp8: '36',                          improvement: '2\u00d7 larger batches',    fp16Num: 18,   fp8Num: 36 },
  { component: 'PagedAttention pages (Stop 12)',        fp16: '5.12 MB/block',           fp8: '2.56 MB/block',               improvement: '2\u00d7 more blocks per GB', fp16Num: 5.12, fp8Num: 2.56 },
  { component: 'P/D transfer (Stop 13)',                fp16: '179 ms',                  fp8: '90 ms',                       improvement: '2\u00d7 faster TTFT',        fp16Num: 179,  fp8Num: 90 },
  { component: 'G2 capacity (Stop 14)',                 fp16: '800 warm conversations',  fp8: '1,600',                       improvement: '2\u00d7 more',               fp16Num: 800,  fp8Num: 1600 },
  { component: 'G3.5 promotion (Stop 14)',              fp16: '192 ms',                  fp8: '96 ms',                       improvement: '2\u00d7 faster retrieval',   fp16Num: 192,  fp8Num: 96 },
  { component: 'Cache hit rate (Stop 14)',              fp16: 'Baseline',                fp8: 'Higher (more fits per tier)', improvement: 'Fewer costly recomputes',    fp16Num: null, fp8Num: null },
