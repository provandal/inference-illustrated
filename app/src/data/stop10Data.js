// Stop 10: Where in the Sequence? — Position and RoPE
//
// Slotted between Layers on Layers (Stop 9) and The Bridge / KV cache (Stop 11)
// so the reader has multi-head attention, the residual stream, layer stacking,
// and the prefill/decode loop already in view by the time we explain RoPE.
//
// Goal: deep treatment of positional encoding with a focus on RoPE (Rotary
// Position Embeddings), the technique used by Llama, GPT, Qwen, DeepSeek, and
// most modern decoder-only models. The K vectors RoPE produces are exactly
// what the KV cache (Stop 11) is about to start storing.
...
export const PAGES = [
  ...
  { id: 'summary',          label: 'Stop 10 at a Glance',            type: 'static'  },
];

export const NARRATIONS = {
  'position-blind':
    '<p><strong>Stop 10: Where in the Sequence?</strong> Across Stops 1\u20137 we built attention end-to-end \u2014 Query, Key, Value, dot product, softmax, blending. But we left a gaping hole. As built so far, attention is <em>permutation-invariant</em>: shuffle the input words and you get the same attention pattern.</p>' +
  ...
  'rotate-idea':
    '<p>RoPE replaces the additive trick with a multiplicative one. Sinusoidal added a position vector to the embedding <em>once</em>, at the input. RoPE never touches the input embedding at all. Instead, <strong>inside every attention layer, after the model has projected the residual-stream input into Q and K, RoPE rotates those Q and K vectors by an angle proportional to the token\u2019s position</strong>. Then the rotated Q and K feed into the dot product as normal. V is left alone.</p>' +
    '<p style="margin-top:0.5em">Two things to lock in. <strong>The rotation is per-layer.</strong> Each layer has its own W_Q and W_K weights, so each layer produces a different Q and K \u2014 and RoPE re-rotates each layer\u2019s Q and K freshly. The <em>rotation function</em> is the same at every layer (same frequencies, same formula); what differs is which Q and K are being rotated. <strong>And the position never lives in the residual stream</strong> \u2014 so it can\u2019t be drowned out by residual writes the way sinusoidal can.</p>' +
    '<p style="margin-top:0.5em">The second consequence is the headline one. When you compute the dot product Q&middot;K of two rotated vectors, the absolute positions <strong>cancel out</strong> \u2014 only the <em>relative</em> position (m \u2212 n) matters. That cancellation is what makes long-context generalization work, and we prove it line by line on the next page.</p>',
  ...
  'rope-and-cache':
    '<p>Here is where RoPE meets infrastructure \u2014 and it is exactly the territory Stop 11 (the next stop, "The Bridge") is about to map out. When the model caches K vectors, what gets stored is the <strong>post-rotation</strong> K. The rotation has already been applied at the token\u2019s absolute position.</p>' +
...
export const FORWARD_LINKS = [
  { stop: 11, label: 'The Bridge \u2014 KV cache intro', why: 'Immediately next. Formal introduction to the KV cache. The K vectors it stores are exactly the post-rotation K we just built here.' },
  { stop: 12, label: 'KV cache memory',     why: 'How much cache fits per GPU, and what shapes it. The RoPE rotation does not change cache size, but it does shape the bandwidth pattern.' },
