...
function WorkedExamplePage() {
  ...
  return (
    <div>
      ...
      <Callout
        type="note"
        message="<strong>4 dimensions for clarity; real models use many more.</strong> Llama-3&rsquo;s embeddings have 8,192 numbers per token, but we&rsquo;ll see in Stop 9 that each attention head works with a slice of that full vector &mdash; 128 dimensions per head. Other models may use 64, 96, or different values per head. The computation is identical regardless &mdash; multiply corresponding elements, sum the results. More dimensions means more signal for the model to encode subtle distinctions, but the operation itself does not change."
      />
    </div>
  );
}
