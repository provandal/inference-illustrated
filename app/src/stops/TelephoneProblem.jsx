...
function Callout({ type, message }) {
  const styles = {
    note: 'bg-[var(--color-blue-bg)] text-[var(--color-blue-text)] border-l-[var(--color-blue)]',
    warn: 'bg-[var(--color-red-bg)] text-[var(--color-red-text)] border-l-[var(--color-red)]',
    good: 'bg-[var(--color-teal-bg)] text-[var(--color-teal-text)] border-l-[var(--color-teal)]',
  };

  return (
    <div
      className={`px-3.5 py-2.5 text-[13px] leading-relaxed my-3.5 border-l-[3px] ${styles[type]}`}
...
function SectionLabel({ children }) {
  return (
    <div className="text-[11px] font-medium text-[var(--color-text-muted)] uppercase tracking-wider mb-1.5">
      {children}
    </div>
  );
}
...
function HiddenStateBar({ concept, value, isKey, isDark }) {
  return (
    <div className="flex items-center gap-2 my-[3px] text-xs">
      ...
      <div className="flex-1 h-3.5 bg-[var(--color-surface-alt)] rounded-[3px] overflow-hidden">
        <div
            ...
            <div className="flex flex-col items-center gap-0.5">
              <div
                className={`w-[34px] h-[34px] rounded-md flex items-center justify-center
                            ...
                            ${
                              j < currentIndex
                                ? 'border border-[var(--color-border)] bg-[var(--color-surface-alt)] text-[var(--color-text-secondary)]'
...
function CompactUpdatePill({ wordIndex }) {
  return (
    <div className="flex items-center gap-1.5 my-2.5 text-xs text-[var(--color-text-secondary)] flex-wrap justify-center">
      <span className="px-2 py-[3px] rounded bg-[var(--color-surface-alt)] border border-[var(--color-border-light)]">
        h<sub>{wordIndex}</sub>
        {wordIndex === 0 ? ' (zeros)' : ''}
      </span>
        ...
        {ATTENTION_DATA.map(([label, value], i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 my-1 text-xs">
              ...
              <div className="flex-1 h-[18px] bg-[var(--color-surface-alt)] rounded-[3px] overflow-hidden">
                <div
                  className="h-full rounded-[3px] transition-[width] duration-500"
...
export default function TelephoneProblem() {
  ...
  if (isMech1) counterText = 'word 1 of 15 — the hidden state';
  else if (isMech2) counterText = 'word 1 of 15 — the update rule';
  else if (isDone) counterText = 'all 15 words processed';
  else if (isAttention) counterText = 'attention view';
  else if (isWord)
    counterText = `RNN processing: word ${wordIndex + 1} of 15`;
  ...
  return (
    <div>
      ...
      <div className="min-h-[400px]">
        ...
        {isMech2 && (
          <div>
            <div className="font-mono text-[13px] px-3.5 py-2 bg-[var(--color-surface-alt)] rounded-md my-2 text-[var(--color-text)] text-center">
              h<sub>new</sub> = f( <strong>W</strong> × h<sub>old</sub> +{' '}
              <strong>U</strong> × word )
            </div>

            <div className="flex items-center justify-center gap-2 my-3.5 flex-wrap">
              <div className="px-2.5 py-1.5 rounded-md text-xs text-center leading-snug bg-[var(--color-surface-alt)] border border-[var(--color-border-light)] text-[var(--color-text-secondary)] min-w-[80px]">
                <div>
                  h<sub>old</sub>
                </div>
        ...
        {isDone && (
          <div>
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {WORDS.map((word, j) => (
                  <div key={j} className="flex items-center gap-1.5">
                    ...
                    <div className="flex flex-col items-center gap-0.5">
                      <div className="w-[34px] h-[34px] rounded-md flex items-center justify-center text-[9px] font-medium font-mono border border-[var(--color-border)] bg-[var(--color-surface-alt)] text-[var(--color-text-secondary)]">
                        h{j + 1}
                      </div>
                      <span className="text-[9px] text-[var(--color-text-muted)] max-w-[40px] text-center overflow-hidden text-ellipsis whitespace-nowrap">
                        {word}
            ...
            <div className="mt-3.5">
              <SectionLabel>Final hidden state</SectionLabel>
            ...
            <div className="mt-4 text-center">
              <button
                onClick={() => go(1)}
                className="px-6 py-2 text-sm bg-[var(--color-primary)] text-white
