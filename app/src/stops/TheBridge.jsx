...
const NARRATIONS = {
  intro:
    '<strong>Stop 11: And Now, The Cache &mdash; The Bridge.</strong> Over ten stops we assembled the transformer from the ground up &mdash; attention, multi-head, layers, residual stream, and most recently Rotary Position Embeddings. At every step, one structure has been growing in the background: the KV cache. Now it is time to bring it to the front, trace how inference actually works step by step, and confront what the cache costs.',
...
function IntroPage() {
  return (
    <div>
      <Panel>
        <PanelHeader>Ten stops, one structure</PanelHeader>
        <div className="p-4 space-y-3 text-[13px] text-[var(--color-text-secondary)] leading-relaxed">
          <p>
            ...
            <strong className="text-[var(--color-text)]">Rotary Position
            Embeddings</strong> close the position-blindness gap (Stop 10) and
            shape the K vectors that the cache is about to start holding.
          </p>
          <p>
            At every step, K and V vectors were being cached. We first met the
            KV cache in Stop 3, when we learned that K and V are persistent while
            Q is ephemeral. We calculated its per-token size in Stop 8. We saw
            it multiply across layers in Stop 9. And in Stop 10 we saw what
            actually lives inside those cached K vectors: position-rotated
            slices.
          </p>
          <p>
