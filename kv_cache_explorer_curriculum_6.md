...
**Core concept:** Scale by √d, then softmax (exponentiate, sum, divide) to get probabilities.
**Animation type:** Step-by-step arithmetic + temperature slider widget.

#### Key concepts to introduce
- **Softmax** — the function that converts any list of numbers into positive values
  summing to 1, by exponentiating and normalizing
- **Temperature** — a scaling parameter controlling how peaked (focused) or flat
  (exploratory) the softmax distribution is; connects to the temperature slider
  in ChatGPT/Claude API

#### Flow (7 steps)

##### Step 0 — "Where we are in the pipeline"
Pipeline bar with steps 3-4 highlighted. Narration:
> We have raw dot product scores from Stop 5. We need proper attention weights.
> This stop covers the two transformations: scaling (÷ √d) and softmax.

##### Step 1 — "Why raw scores won't work"
Table of raw scores. Narration explains two problems:
- Negative values (can't blend with negative weights)
- Don't sum to 1 (need proper weighted average)

##### Step 2 — "Scaling: dividing by √d"
Table: raw scores → ÷ √d → scaled scores. Completing the preview from Stop 5.
Note: with 4D toy vectors the scaling is modest; with 128D real vectors,
scores of 30+ get tamed to ~2-3.

##### Step 3 — "Softmax: step by step"
The core computation, walked through completely:

**Step A — Exponentiate:** Each scaled score → e^score.
Explain WHY the exponential:
> The exponential function e^x has two critical properties:
> - Always positive output, even for negative inputs (solves Problem 1)
> - Amplifies differences: slightly higher input → disproportionately higher
>   output (creates useful "winner-take-more" behavior)

**Step B — Sum** all exponentials.

**Step C — Divide** each by the sum → attention weights (percentages).

Full worked example with all 8 words from our sentence.

Note on precision:
> Mathematically, the division guarantees the weights sum to exactly 1.0.
> In practice, computer arithmetic may produce 0.99999998 or 1.00000001
> due to floating point precision — this is negligible and has no effect
> on the model's behavior. Neural network math is approximate by nature,
> unlike storage protocols where bit-level precision matters.

Note on distribution shape:
> With our 4-dimensional toy vectors, "storage controller" gets 17.4% —
> not dramatically more than others. This is because small-dimensional
> vectors produce modest score differences. With real 128-dimensional
> vectors, score differences are larger, and softmax produces much more
> peaked distributions: "storage controller" might get 48% while "last"
> gets 2%.

Visual: Three-stage animated calculation. Scores flow through e^x boxes,
results sum, each divides by sum, final percentages appear as bars.

Term defined: **softmax**

##### Step 4 — "Why the exponential? Why not just normalize?"
Two reasons:
1. Handling negatives (exponential guarantees positivity)
2. Amplifying the winner (exponential is nonlinear — 1.0 difference in input
   becomes 2.7× ratio in output, making the model focus on truly relevant words)

Optional calculus expandable:
> **For those who want the deeper math:** The exponential function has a
> remarkable property: its derivative is itself (d/dx e^x = e^x). This
> means the gradient of softmax has an elegant closed-form expression,
> making backpropagation through the softmax layer computationally clean.
> This isn't just aesthetic — when you're computing gradients for billions
> of parameters across trillions of training examples, computational
> elegance translates directly to feasibility. Other positive-valued
> functions could work in principle (x², |x|), but their gradient
> properties make training less stable. The exponential is both
> mathematically beautiful and practically essential.

##### Step 5 — "Temperature: focus vs. creativity"
softmax(scores / temperature).
- Low temperature (0.1): scores × 10, very peaked, one word dominates
- Temperature 1.0: default, natural distribution
- High temperature (2.0+): scores shrunk, flatter, more exploratory
- Temperature → 0: all attention on single highest word
- Temperature → ∞: equal attention everywhere

Connection to user experience:
> If you've adjusted the temperature setting when using ChatGPT or Claude's
> API, you've been controlling this exact mechanism — not in attention
> specifically, but in the final token prediction step, which uses the same
> softmax function. The principle is identical: temperature controls the
> tradeoff between focus and exploration.

Visual: Interactive temperature slider (0.1 to 3.0). Our 8 attention scores
as bars, updating in real time. At low temperature, one bar dominates. At
high temperature, bars equalize. Numerical values update live.

Term defined: **temperature**

##### Step 6 — Bridge to Stop 7
Narration:
> We now have proper attention weights — positive values summing to 1.
> "Storage controller" gets the most attention from "faulty," with the
> remaining attention distributed across other words.
>
> The final step: use these weights to extract information. Multiply each
> word's **Value** vector by its weight and sum them all together. The
> result is a new vector for "faulty" that's blended with context from
> the words it attended to — a representation that *knows* "faulty"
> refers to the storage controller.
>
> Stop 7 completes the attention mechanism.

#### Design notes
- Include rounding/precision note in Step 3 — addresses engineering instinct
  about exactness while explaining that neural network math is inherently
  approximate.
- Calculus expandable in Step 4 is optional — available for those who want
  deeper mathematical grounding, not required for the main flow.
- Temperature slider (Step 5) is a highlight — connects abstract math to
  a feature every LLM user has encountered.
...
**Core concept:** The weighted sum blends Value vectors according to attention weights,
producing a context-enriched representation. The residual connection preserves the original.
**Animation type:** Paint-mixing animation + residual connection flow diagram.

#### Key concepts to introduce
- **Weighted sum** — multiplying each Value by its attention weight and summing;
  produces a blend dominated by the highest-weighted words
- **Context-enriched representation** — a word's meaning after being informed by
  the words it attended to; "faulty" becomes "faulty-as-describing-the-controller"
- **Residual connection (skip connection)** — adding the attention output to the
  original embedding rather than replacing it; preserves original information and
  enables training of deep networks. From ResNet (He et al., 2015)

#### Flow (7 steps)

##### Step 0 — "The final step"
Pipeline bar with step 5 highlighted.
Narration:
> We're at the last step. We have attention weights telling us how much "faulty"
> should listen to each word. Now we use those weights to extract information
> and produce a new, context-enriched representation.

##### Step 1 — "The vectors we're blending"
One job: show the user the actual Q and V vectors they're about to see blended.

Narration:
> Before we blend, let's see what we're working with. Here's "faulty's" Query
> and the Value vectors for some key words (simplified to 4 dimensions):
>
> "faulty" — Q = [0.8, -0.3, 0.5, 0.2]
>   (encoding: "I'm an adjective looking for the noun I describe")
>
> And the Values that "faulty" will be blending from:
>
> "storage" — V = [0.7, 0.1, -0.3, 0.5] (carries: hardware infrastructure)
> "controller" — V = [0.9, -0.4, 0.7, 0.3] (carries: manageable component)
> "crashed" — V = [-0.3, 0.8, 0.1, -0.5] (carries: failure event)
> "technician" — V = [-0.2, 0.8, 0.1, 0.6] (carries: human actor)
> "last" — V = [-0.1, -0.3, 0.4, 0.8] (carries: temporal reference)
>
> These are the ingredients. The attention weights from Stop 6 determine
> the proportion of each ingredient in the final blend.

Visual: Table showing word, Value vector, and conceptual interpretation.
Clean, compact — setting up the blending animation in Step 2.

##### Step 2 — "Blending: a visual intuition"
Visual-first step. The narration sets up the metaphor; the animation does the work.

Narration:
> The **weighted sum** takes each word's Value vector, scales it by that
> word's attention weight, and adds all the scaled vectors together.
>
> Think of it like mixing paint. Each word is a different color. The
> attention weights determine how much of each color you pour into the
> mix. "Storage" and "controller" have the highest weights — they contribute
> the most to the final color. "Last" has the lowest weight — just a trace.
>
> The result is a single blended vector, dominated by the words that received
> the most attention.

Visual: Animation showing 15 Value vectors as colored bars. Each bar scales
down proportionally to its attention weight (high-weight words stay tall,
low-weight words shrink). All scaled bars merge into a single output bar
whose composition reflects the blend — visually dominated by the colors
of the high-attention words.

Optional expandable: "See the arithmetic" — shows the element-by-element
multiplication and summation for one dimension as a worked example. Available
for those who want numbers, not required for understanding.

Term defined: **weighted sum**

##### Step 3 — "What 'context-enriched' actually means"
Narration:
> Before attention, "faulty" was represented by its embedding — a generic
> vector meaning "broken, defective, not working properly." It had no idea
> what was faulty.
>
> After the weighted sum, "faulty" has a new representation blended with
> the Values of the words it attended to. It now carries traces of "storage"
> (hardware domain), "controller" (specific component), "crashed" (failure
> context) — each weighted by relevance.
>
> The output is no longer generic "faulty." It's "faulty as it appears in
> this specific sentence, describing the storage controller that was replaced."
>
> This is why the same word means different things in different sentences.
> "Faulty" in "the faulty wiring" would produce a completely different output
> — attending to "wiring" instead of "controller," blending different Values.
> The word is the same; the context-enriched representation is unique to
> each sentence.

Visual: Side-by-side.
Left: "faulty" before attention — single-color bar, labeled "generic: defective."
Right: "faulty" after attention — multi-colored bar showing blended contributions,
visually dominated by the colors of "storage" and "controller."

Term defined: **context-enriched representation**

##### Step 4 — "The residual connection: add, don't replace"
Narration:
> There's one more architectural detail that's essential. The attention output
> is NOT the new representation directly. Instead, it's **added to** the
> original embedding:
>
> final = original embedding + attention output
>
> This is called a **residual connection** (or **skip connection**), from a
> 2015 breakthrough in image recognition by Kaiming He called **ResNet**.
> The idea: let the original signal pass through untouched, and add new
> information on top — like writing annotations in the margin without erasing
> the original text.
>
> **Why add instead of replace?**
>
> First, it's **safer**. If the attention mechanism produces a poor result,
> the original information survives. With replacement, bad attention could
> destroy useful information. With addition, the worst case is noise on
> top of signal.
>
> Second, it solves the **vanishing gradient problem** — the same issue that
> killed the RNN in Stop 1. In a transformer with 80 layers, gradients must
> flow backward through every layer during training. Without residual
> connections, the gradient degrades at each layer — same exponential decay
> we watched in the hidden state. Residual connections provide a direct
> path — a "gradient highway" — that keeps the training signal strong
> across all 80 layers.
>
> Without residual connections, transformers couldn't be trained at the
> depths that make them powerful.

Visual: Flow diagram. Original vector → splits into two paths. Path A goes
through the attention mechanism (all five pipeline steps). Path B goes
straight through (skip connection, drawn as a clean direct line). Both paths
meet at a "+" node. Path B labeled "preserves original." Path A labeled
"adds context."

Term defined: **residual connection (skip connection)**

##### Step 5 — "The complete attention pipeline"
The payoff: seeing all five steps as one unified flow.

Narration:
> Let's see everything from Stops 3 through 7 as one complete flow. This is
> one attention computation for one word:

Visual: Definitive reference diagram. Clean animated flow:
- Word enters as embedding
- Embedding splits three ways through W_Q, W_K, W_V
- Q goes to the matching step
- K and V flow into a prominently labeled "KV cache" box
- Q · each cached K → scores
- Scores ÷ √d → scaled scores
- Softmax → attention weights
- Weights × cached Values → attention output
- Attention output + original embedding (residual) → final representation

Every stage labeled. The KV cache box is visually prominent — K and V flowing
in, being stored, then being read back during matching and blending. This
makes the cache's role in the pipeline tangible.

Narration:
> This is one attention "head" in one layer, using one set of W_Q, W_K, W_V.
> The model computes this for every word simultaneously — not sequentially
> like the RNN. That's the parallelism that makes transformers fast.
>
> But one head learns one type of attention pattern. Language has many
> simultaneous relationships — syntax, semantics, coreference, proximity.
> What if the model ran many of these computations in parallel, each with
> its own weight matrices, each discovering a different kind of pattern?

##### Step 6 — Bridge to Stop 8
Narration:
> That's **multi-head attention**. Instead of one attention computation, the
> model runs many — typically 32 to 128 — in parallel. Each head has its own
> W_Q, W_K, W_V and discovers its own specialization.
>
> Multi-head attention is where the model gets its depth of understanding.
> It's also where the KV cache gets its size: each head stores its own K and
> V vectors for every token. A model with 64 heads stores 64 K vectors and
> 64 V vectors per token, per layer. Multiply across 80 layers and the cache
> grows to tens of gigabytes.
>
> Stop 8 shows how multiple heads work together and why they specialize.

#### Design notes
- The weighted sum is shown through ANIMATION (paint mixing), not text arithmetic.
  Arithmetic is available as an optional expandable for those who want it.
- "Storage" and "controller" are treated as separate words throughout, consistent
  with the rest of the curriculum.
- The residual connection ties back to Stop 1's vanishing gradient problem,
  making it feel like part of the ongoing story, not a tangent.
- The complete pipeline diagram (Step 5) is the definitive reference visual
  for the attention mechanism. KV cache is prominently shown in the flow.
...
| 18 | Scaling factor (√d) | Divides dot products by √dimension to prevent softmax saturation; the "scaled" in scaled dot-product attention | Stop 6 |
| 19 | Softmax | Converts any list of numbers into positive values summing to 1: exponentiate each, divide by total. The exponential amplifies score differences | Stop 6 |
| 19b | Temperature | Scaling factor before softmax controlling peaked (focused) vs. flat (exploratory) distributions; same mechanism behind LLM API temperature settings | Stop 6 |
